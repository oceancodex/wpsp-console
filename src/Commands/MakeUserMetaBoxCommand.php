...
class MakeUserMetaBoxCommand extends Command {
	...
	protected function execute(InputInterface $input, OutputInterface $output): int {
		...
		if (!$id) {
			...
			if (empty($id)) {
				$output->writeln('Missing ID for the user meta box. Please try again.');
		...
		// Check exist.
		$exist = FileSystem::exists($this->mainPath . '/app/Extras/Components/UserMetaBoxes/' . $idSlugify . '.php');
		$exist = $exist || FileSystem::exists(__DIR__ . '/../../../resources/views/modules/user-meta-boxes/'. $idSlugify . '.blade.php');
		if ($exist) {
			$output->writeln('[ERROR] User meta box: "' . $id . '" already exists! Please try again.');
		...
		FileSystem::put($this->mainPath . '/app/Extras/Components/UserMetaBoxes/' . $idSlugify . '.php', $content);
		...
		// Output message.
		$output->writeln('Created new user meta box: "' . $id . '"');
