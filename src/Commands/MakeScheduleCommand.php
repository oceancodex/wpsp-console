...
class MakeScheduleCommand extends Command {
	...
	protected function execute(InputInterface $input, OutputInterface $output): int {
		...
		if (!$hook) {
			...
			if (empty($hook)) {
				$output->writeln('Missing hook for the schedule. Please try again.');
		...
		// Check exist.
		$exist = FileSystem::exists($this->mainPath . '/app/Extend/Components/Schedules/' . $hookSlugify . '.php');
		if ($exist) {
		...
		FileSystem::put($this->mainPath . '/app/Extend/Components/Schedules/' . $hookSlugify . '.php', $content);
